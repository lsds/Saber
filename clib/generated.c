...
JNIEXPORT jint JNICALL Java_uk_ac_imperial_lsds_saber_devices_TheCPU_optimisedDistinct
  (JNIEnv * env, jobject obj, jobject buffer, jint bufferSize, jint bufferStartPointer, jint bufferEndPointer,
   ...
   jint mapSize) {
    ...
    ht_node<int, int> *openingWindowsResults = (ht_node<int, int> *) env->GetDirectBufferAddress(openingWindowsBuffer); // the results here are in the
    ht_node<int, int> *closingWindowsResults = (ht_node<int, int> *) env->GetDirectBufferAddress(closingWindowsBuffer); // form of the hashtable
    ht_node<int, int> *pendingWindowsResults = (ht_node<int, int> *) env->GetDirectBufferAddress(pendingWindowsBuffer);
    ...
    hashtable<int, int> map(mapSize);
    ht_node<int, int> *hTable = map.getTable();
    ...
    bool hasComplete = ((data[bufferEndPointer - 1].timestamp - data[bufferStartPointer].timestamp) / windowPaneSize) >=
                       panesPerWindow;
    int firstPane = data[bufferStartPointer].timestamp / windowPaneSize;
    ...
    if (streamStartPointer == 0) {
        ...
        while (currPos < bufferEndPointer) {
            ...
            map.insert_and_increment_counter(&data[currPos].vehicle, data[currPos].timestamp); //ht_insert_and_increment(hTable, data[currPos].vehicle, 1, data[currPos].timestamp, mapSize);
            currPos++;
        }
        ...
        tempPane = (bufferStartPointer != 0) ? data[bufferStartPointer - 1].timestamp / windowPaneSize :
                   data[bufferSize / sizeof(PosSpeedStr) - 1].timestamp / windowPaneSize;
        while (currPos < bufferEndPointer) {
            map.insert_and_increment_counter(&data[currPos].vehicle, data[currPos].timestamp); //ht_insert_and_increment(hTable, data[currPos].vehicle, 1, data[currPos].timestamp, mapSize);
            activePane = data[currPos].timestamp / windowPaneSize;
        ...
        tempPane = (bufferStartPointer != 0) ? data[bufferStartPointer - 1].timestamp / windowPaneSize :
                   data[bufferSize / sizeof(PosSpeedStr) - 1].timestamp / windowPaneSize; // TODO: fix this!!
        ...
        while (currPos < bufferEndPointer) {
           ...
           // Does the second check stand???
            if (activePane - tempPane >= panesPerSlide &&
                ...
                std::memcpy(closingWindowsResults + closingWindowsPointer, hTable, mapSize * sizeof(ht_node<int, int>));
        ...
        if (completeWindows > 0 && !startingFromPane) {
            for (int i = bufferStartPointer; i < startPositions[1]; i++ ) {
                map.evict_and_decrement_counter(&data[i].vehicle);
            }
        }
    }
    ...
    if (streamStartPointer != 0 && !hasComplete) {  // (completeWindows == 0 && closingWindows == 0) {
        // write results
        std::memcpy(pendingWindowsResults + pendingWindowsPointer, hTable, mapSize * sizeof(ht_node<int, int>));
    ...
    //} else if (completeWindows == 0  && currentSlide > 1 && data[startPositions[0]].timestamp%windowSlide==0) {
        ...
        map.insert_and_increment_counter(&data[currPos].vehicle, data[currPos].timestamp);
        currPos++;
        ...
        removalIndex = (startingFromPane) ? currentWindow : currentWindow + 1;
        while (currPos < bufferEndPointer) {
            // remove previous slide
            tempStartPosition = startPositions[removalIndex - 1];
            tempEndPosition = startPositions[removalIndex];

            //printf("tempStartPosition %d, tempEndPosition %d \n", tempStartPosition, tempEndPosition);
            //fflush(stdout);
            for (int i = tempStartPosition; i < tempEndPosition; i++) {
                map.evict_and_decrement_counter(&data[i].vehicle); //ht_update_counter(hTable, data[i].vehicle, -1, mapSize);
            }
    ...
    free(startPositions);
    free(endPositions);
    map.deleteHashtable(); //ht_free(map);


    // return the variables required for consistent logic with the Java part
    arrayHelper[0] = openingWindowsPointer * sizeof(ht_node<int, int>);
    arrayHelper[1] = closingWindowsPointer * sizeof(ht_node<int, int>);
    arrayHelper[2] = pendingWindowsPointer * sizeof(ht_node<int, int>);
...
JNIEXPORT jint JNICALL Java_uk_ac_imperial_lsds_saber_devices_TheCPU_optimisedAggregateHashTables
  ...
  jobject openingWindowsBuffer, jobject completeWindowsBuffer, jint resultBufferPosition) {
    ...
    // Input Buffers
    ht_node<int, int> *buffer1= (ht_node<int, int> *) env->GetDirectBufferAddress(buff1);
    ht_node<int, int> *buffer2= (ht_node<int, int> *) env->GetDirectBufferAddress(buff2);
    hashtable<int, int> map1 (buffer1, mapSize);
    hashtable<int, int> map2 (buffer2, mapSize);
    ...
    // Output Buffers
    ht_node<int, int> *openingWindowsResults;
    DistinctRes *completeWindowsResults; // the results here are packed

    if (!pack) {
        openingWindowsResults = (ht_node<int, int> *) env->GetDirectBufferAddress(openingWindowsBuffer);
    ...
    for (int idx = start1; idx < end1; idx++) {
        ...
        isFound = map2.get_index(&buffer2[start2], &buffer1[idx].key, posInB2); //ht_get_index(&buffer2[start2], buffer1[idx].key, mapSize, posInB2);
        if (posInB2 < 0) {
    ...
    for (int idx = start2; idx < end2; idx ++) {
        ...
        if (pack) {
            ...
            /* Create a new hash table entry */
            isFound = map2.get_index(&openingWindowsResults[resultIndex], &buffer2[idx].key, posInRes); //ht_get_index(&openingWindowsResults[resultIndex], buffer2[idx].key, mapSize, posInRes);
    ...
    // return the variables required for consistent logic with the Java part
    return (pack) ? resultIndex*sizeof(DistinctRes) : (resultIndex+mapSize)*sizeof(ht_node<int, int>) ;
}
