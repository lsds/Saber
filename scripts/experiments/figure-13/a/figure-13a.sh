...
# Figure 13a: Performance impact of query task size for different window sizes and slides
...
#
F="$SABER_HOME/scripts/experiments/figure-13/a"
major=
minor=
...
saberExperimentSetup () {
	...
	#
	# Window settings
	OPTS="$OPTS --window-type row --window-size 1 --window-slide 1"
...
saberExperimentRun () {
	#
	# for N in 0.03125 to 4096 KB/batch
	# do 
	# 	Run Selection CPU-only with task size $N (window 1 row, slide 1 row) (N-1)
	# 	Run Selection GPU-only with task size $N (window 1 row, slide 1 row) (N-2)
	# done
	#
	errors=0
	points=0
	...
	for mode in "cpu" "gpu"; do
		...
		--batch-size $N --intermediate-buffer-size $N --execution-mode $mode
...
saberExperimentCheckResults () {
	...
	DATDIR="$ORIGIN/figure-13/selection/tumbling-window/1-1"
	#
	# Data format:
	#
	#       x         MIN         AVG    ...
	...
	for mode in "cpu" "gpu"; do
		...
		printf "Figure %3s: line %-30s: x %7d: " "$FIGID" "$line" "$N"
...
[ $errors -eq 0 ] && {
	...
	[ $? -eq 0 ] && {
		...
		if [ ! -z $SABER_MAJOR_VERSION ]; then # Was the major version set?
			...
			P="$DATDIR/figure-13/selection/tumbling-window/1-1"
