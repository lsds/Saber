...
USAGE="usage: selection.sh [ --batch-size ] [ --window-type ] [ --window-size ] [ --window-slide ] [ --input-attributes ] [ --comparisons ] [ --selectivity ] [ --tuples-per-insert ]"

CLS="uk.ac.imperial.lsds.saber.experiments.microbenchmarks.TestSelection"
...
if [ -z "$SABER_CONF_EXPERIMENTDURATION" ]; then
	...
	"$SABER_HOME"/scripts/run.sh --mode background --alias "selection" --class $CLS --duration $duration -- $SYSARGS $APPARGS
	errorcode=$?
fi
