...
USAGE="usage: noop.sh [ --batch-size ] [ --window-type ] [ --window-size ] [ --window-slide ] [ --input-attributes ] [ --tuples-per-insert ]"

CLS="uk.ac.imperial.lsds.saber.experiments.microbenchmarks.TestNoop"
...
if [ -z "$SABER_CONF_EXPERIMENTDURATION" ]; then
	...
	"$SABER_HOME"/scripts/run.sh --mode background --alias "noop" --class $CLS --duration $duration -- $SYSARGS $APPARGS
	errorcode=$?
fi
