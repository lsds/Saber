...
CLS="uk.ac.imperial.lsds.saber.experiments.microbenchmarks.scheduling.W3"
...
if [ -z "$SABER_CONF_EXPERIMENTDURATION" ]; then
	...
	"$SABER_HOME"/scripts/run.sh --mode background --alias "w3" --class $CLS --duration $duration -- $SYSARGS $APPARGS
	errorcode=$?
fi
