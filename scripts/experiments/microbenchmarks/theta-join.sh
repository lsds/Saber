...
USAGE="usage: theta-join.sh [ --batch-size ] [ --window-type-of-first-stream ] [ --window-size-of-first-stream ] [ --window-slide-of-first-stream ] [ --input-attributes-of-first-stream ] [...] [ --comparisons ] [ --selectivity ] [ --tuples-per-insert ]"

CLS="uk.ac.imperial.lsds.saber.experiments.microbenchmarks.TestThetaJoin"
...
# Number of input attributes of second stream (default is 6)
INPUT_ATTRS2=

# Number of predicate comparisons (default is 0)
COMPARISONS=

# Selectivity (default is 1%)
SELECTIVITY=
...
do
	case "$1" in
		...
		;;
		--selectivity)
		saberOptionIsIntegerWithinRange "$1" "$2" "0" "100" || exit 1
		SELECTIVITY="$2"
		shift 2
		;;
		--tuples-per-insert)
...
[ -n "$INPUT_ATTRS2" ] && \
APPARGS="$APPARGS --input-attributes-of-second-stream $INPUT_ATTRS2"

[ -n "$COMPARISONS" ] && \
APPARGS="$APPARGS --comparisons $COMPARISONS"

[ -n "$SELECTIVITY" ] && \
APPARGS="$APPARGS --selectivity $SELECTIVITY"
...
if [ -z "$SABER_CONF_EXPERIMENTDURATION" ]; then
	...
	"$SABER_HOME"/scripts/run.sh --mode background --alias "thetajoin" --class $CLS --duration $duration -- $SYSARGS $APPARGS
	errorcode=$?
fi
